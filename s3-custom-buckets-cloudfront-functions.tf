resource "aws_cloudfront_function" "custom_s3_buckets_viewer_request" {
  for_each = {
    for k, v in local.custom_s3_buckets : k => v if v["cloudfront_s3_root"] != null || v["cloudfront_basic_auth_user_list"] != null || v["cloudfront_infrastructure_ecs_cluster_service_path"] != null || v["cloudfront_s3_root_file"] != null
  }
  ...
  code = templatefile("${path.root}/cloudfront-functions/viewer-request.js.tpl", {
    ...
    root_file             = each.value["cloudfront_s3_root_file"] != null ? each.value["cloudfront_s3_root_file"] : ""
  })
}
