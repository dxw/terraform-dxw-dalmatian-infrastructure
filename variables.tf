...
variable "infrastructure_cognito_user_pools" {
  description = <<EOT
    ...
    The pool schema is immutable after creation, so `custom_attributes` is applied on create only (`ignore_changes = [schema]`); adding an attribute later means a new pool.
    {
      pool-name = {
        deletion_protection: Enable deletion protection on the pool (default true)
        password_minimum_length: Minimum password length (default 16, dxw policy). Upper case, lower case and numbers are always required
        ...
        threat_protection: `OFF` (default), `AUDIT` or `ENFORCED`. Anything other than `OFF` moves the pool to the Plus feature plan, which is charged per monthly active user
  ...
  validation {
    condition = alltrue(flatten([
      for k, v in var.infrastructure_cognito_user_pools : [
        for ck, cv in coalesce(v["clients"], {}) : alltrue([
          for flow in coalesce(cv["explicit_auth_flows"], []) : startswith(flow, "ALLOW_")
        ])
      ]
    ]))
    error_message = "Every entry in explicit_auth_flows must be an ALLOW_* flow name."
  }
