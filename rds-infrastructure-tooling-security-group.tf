resource "aws_security_group" "infrastructure_rds_tooling" {
  ...
  description = "Infrastructure RDS tooling"
  vpc_id      = aws_vpc.infrastructure[0].id
}

resource "aws_security_group_rule" "infrastructure_rds_tooling_egress_https_tcp" {
  for_each = local.enable_infrastructure_rds_tooling ? local.infrastructure_rds : {}
  ...
  security_group_id = aws_security_group.infrastructure_rds_tooling[each.key].id
}

resource "aws_security_group_rule" "infrastructure_rds_tooling_egress_https_udp" {
  for_each = local.enable_infrastructure_rds_tooling ? local.infrastructure_rds : {}
...
resource "aws_security_group_rule" "infrastructure_rds_tooling_egress_dns_tcp" {
  for_each = local.enable_infrastructure_rds_tooling ? local.infrastructure_rds : {}
...
resource "aws_security_group_rule" "infrastructure_rds_tooling_egress_dns_udp" {
  for_each = local.enable_infrastructure_rds_tooling ? local.infrastructure_rds : {}
...
resource "aws_security_group_rule" "infrastructure_rds_tooling_egress_rds" {
  for_each = local.enable_infrastructure_rds_tooling ? local.infrastructure_rds : {}
