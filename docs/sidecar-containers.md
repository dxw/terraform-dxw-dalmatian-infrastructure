...
Look for the line `resolved=<registry>/<image>@sha256:<digest> os=linux arch=amd64`
and the closing `describe-images` JSON showing the ECR image digest, tag and
push time. The resolved digest is the source manifest that was pulled; the
ECR `imageDigest` is the digest of the single-platform copy stored in ECR and
will differ from a multi-platform source index.
